...
- **Город.** 42 311 зданий на реальных местах. У 46% высота или этажность из OSM, у остальных оценка по типу и площади с флагом `EstimatedHeight`. Ещё 33 768 дорог, вода, парки, 560 тыс. деревьев с LOD, эстакада LRT и 9 ориентиров ручной работы: Байтерек, Хан Шатыр, Пирамида, Нұр Әлем, купол Акорды, Хазрет Султан, Қазақ Елі, «Северное сияние», Астана Арена.
- **Цветовые акценты** (`BuildingAccent` в `schema.ts`, считаются в `scripts/city/lib/classify.ts`): стеклянные фасады высоток, изумрудное стекло Изумрудного квартала, абрикосовые крыши школ и детсадов, коралловые крыши больниц и поликлиник, терракотовые беговые дорожки. Помогают быстро найти на карте школы (M7) и поликлиники (M8).
- **Жизнь города** (`src/scene/world/Traffic.tsx`): машины и автобусы по реальному графу магистралей с правосторонним движением, поезда LRT на эстакаде. Это декоративный трафик, не модель пробок: для мер M1/M2 его можно раскрасить или ускорить.
...
Визуал дорабатывается параллельно в `src/scene/world/*`, `src/scene/fx/*`, `src/scene/palette.ts`, `src/scene/markers/*` и `src/scene/camera/*`. Уже сделано: тени от солнца, дороги с минимальной экранной шириной на карте, затухание дальних колец в 3D, минимальный экранный размер колец, акценты зданий, трафик и LRT. Твоя зона: `src/sim/`, `src/domain/`, `src/ui/`, `src/app/api/` и одна строка подключения слоя в `CityCanvas.tsx`. Правки `src/city/store.ts` согласуй: это общий контракт.
