...
4. Нажмите на метку **Нура** → **«Смотреть в 3D»** и добавьте меры прямо из карточки района. Или слева: **«Быстрый старт → Пример из кейса»**. Затем **«Подписать бюджет и прожить 2 года»**.
...
![3D-вид района Нура: карточка слева показывает слабые места района и меры, которые их закрывают, с кнопкой «В план»; школа из плана уже стоит на карте](docs/media/decide-3d.png)

- **Карточка района в 3D.** Клик по метке района → «Смотреть в 3D». Камера подлетает к району, а карточка показывает:
  - слабые места района (ниже 50, провалы ниже 40 — красным);
  - ЧС, которая ждёт район;
  - до пяти мер, которые закрывают эти места или смягчают ЧС, с ценой, затронутыми показателями и кнопкой **«В план»**.

  Недоступные меры уходят вниз с причиной: превышение бюджета, конфликт, лимит направления. Здание меры сразу появляется на карте. Когда набрано 5 решений, бюджет можно подписать прямо из карточки.
- **ЧС в 3D после подписи.** Клик по значку молнии → «Смотреть в 3D» → выбор реакции: «Не вмешиваться», временные меры или полная ликвидация. Реакция оплачивается из резерва — неподписанного остатка бюджета. Score с учётом ЧС пересчитывается сразу.
...
### Независимая сверка с документом кейса

`npm run sim:verify` ([`scripts/sim/verify-case.ts`](scripts/sim/verify-case.ts)) читает таблицы прямо из [документа кейса](docs/case/akim-simulator-dataset-and-rules.md), а не из кода.
- Сверяет с `data.ts` все 127 чисел: 50 исходных значений, доли населения, веса, стоимость, лаги и эффекты 14 мер, синергии, несовместимости.
- Реализует правила 1–8 и формулу заново, по тексту документа, и прогоняет эталон и движок на всех 1 407 054 планах: все сочетания пяти мер во всех районах, допустимые и недопустимые.
- Результат: одинаковые решения «допустим / не допустим», одинаковые причины отказа, расхождение Score = 0.
- Проверку проверили на ошибках: неверный лаг M7 и синергия, умноженная на лаг, обе ловятся.
...
- **Память народа.** До подписи вы даёте до трёх публичных обещаний, например «Выведу школы Нуры из критической зоны». После подписи код проверяет каждое обещание по показателям. Нарушенное обещание злит район, и AI-анализ его припоминает.
...
npm run sim:verify           # сверка с документом кейса: 127 чисел датасета, 1 407 054 плана против эталона
